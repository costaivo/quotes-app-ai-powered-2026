...
- `app/be/src/quotes/entities/quote.entity.ts` - The TypeORM entity for quotes. This file will be updated to add database indexes to the `author` and `text` columns.
- `app/be/src/database/migrations/` - A new migration file will be created in this directory to apply the index changes to the database schema.
...
- `app/be/test/app.e2e-spec.ts` - The end-to-end test file. It will be updated to include tests for the new filtering functionality.
...
- [ ] 1.0 Update Quote Entity and Database
  - [ ] 1.1 Add `@Index()` decorators to the `author` and `text` properties in `app/be/src/quotes/entities/quote.entity.ts` to improve query performance.
  - [ ] 1.2 Generate a new TypeORM migration to apply the new database indexes. You can use a command like `pnpm --filter be typeorm:migration:generate -d app/be/src/database/database.config.ts app/be/src/database/migrations/AddIndexesToQuotes`.
  - [ ] 1.3 Review the generated migration file to ensure it correctly adds the indexes to the `quote` table.
